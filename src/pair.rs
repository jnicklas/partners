...
impl<'a> AuthorInformation for &'a Pair<'a> {
    fn get_name(&self) -> Cow<str> {
        Cow::Owned(self.authors.iter().map(|a| &*a.name).join(" and "))
    }

    fn get_nick(&self) -> Cow<str> {
        Cow::Owned(self.authors.iter().map(|a| &*a.nick).join(&self.config.separator))
    }

    fn get_email(&self) -> Cow<str> {
        let names = self.authors.iter().map(|a| &*a.nick).join(&self.config.separator);
