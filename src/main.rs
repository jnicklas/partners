...
static CFG: config::ConfigType = config::ConfigType::File("./partners.cfg");
...
fn parse_author_line(config: &Rc<Config>, line: &str) -> Result<Author> {
  ...
  let email = config::get(&CFG, &format!("author.{}.email", nick)[]).ok();

  Ok(Author { config: config.clone(), nick: nick, name: name, email: email })
}

fn get_authors(config: &Rc<Config>) -> Result<Vec<Author>> {
  let lines = try!(config::list(&CFG, "author.\\w+.name"));
  lines.iter().map(|line| parse_author_line(config, &line[])).collect()
}

fn write_author(author: &Author) -> Result<()> {
  try!(config::set(&CFG, &*format!("author.{}.name", author.nick), &*author.name));
  if let Some(ref email) = author.email {
    try!(config::set(&CFG, &*format!("author.{}.email", author.nick), &**email));
  }
  Ok(())
}
...
fn main() {
  ...
  let config = Rc::new(Config {
    domain: config::get(&CFG, "config.domain").unwrap_or_else(|_| "example.com".to_string()),
    prefix: config::get(&CFG, "config.prefix").unwrap_or_else(|_| "dev".to_string()),
    separator: config::get(&CFG, "config.separator").unwrap_or_else(|_| "+".to_string()),
  });
  ...
  if args.get_bool("list") {
    ...
    match get_authors(&config).unwrap().iter().find(|a| a.nick == nick) {
      Some(author) => {
        config::set(&config::ConfigType::Global, "partners.current", &*author.nick).unwrap();
        config::set(&config::ConfigType::Global, "user.name", &*author.name).unwrap();
        config::set(&config::ConfigType::Global, "user.email", &*author.get_email()).unwrap();
        print_current();
      }
      None => {
